...
function joinRoom(socket, room) {
    ...
    if (currentRoom[socket.id] && currentRoom[socket.id] != room){
        socket.broadcast.to(currentRoom[socket.id]).emit('message', { text : nickNames[socket.id] + ' left the ' + room});
    }
    socket.join(room);
    currentRoom[socket.id] = room;
    socket.emit('joinResult',{room:room});
    socket.broadcast.to(room).emit('message', { text : nickNames[socket.id] + ' has joined the ' + room});
    ...
    if (usersInRoom.length >= 1){
        ...
        userInRoomSummary += ',' + nickNames[socket.id] + ' }';
